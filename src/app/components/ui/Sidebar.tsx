...
const Sidebar: React.FC<SidebarProps> = ({ sidebarVisible, setSidebarVisible }) => {
    ...
    const hardcodedGroupedChats: { [key: string]: { id: number; title: string }[] } = {
        "Hoy": [
    ...
    return (
        <div
            className={`flex flex-col w-64 gap-4 bg-tertiary rounded-lg text-white p-4 h-full fixed top-0 left-0 transition-all duration-300 ease-in-out 
            ${sidebarVisible ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
        >
            <Button text="+ New Chat" className="w-full py-1 rounded-lg" style={"SECONDARY"} />
            ...
            <div className="flex flex-col overflow-y-auto gap-1 flex-grow sidebar-scroll">
                {Object.keys(hardcodedGroupedChats).map((date) => (
                    <div key={date} className="flex flex-col gap-1">
                        <div className="text-sm text-foreground-muted font-semibold">{date}</div>
